...
class BigIntegerField:
    def from_bytes(self, b: bytes) -> int:
        return int.from_bytes(b, byteorder="big")

    def to_bytes(self, num: int, length: int = 32) -> bytes:
        return num.to_bytes(length, byteorder="big")

    def serialize(self, num: int) -> str:
        return hex(num)

    def deserialize(self, s: str) -> int:
        return int(s, 16)


class ECDSAPointField:
    def to_bytes(self, point: PointJacobi) -> bytes:
        return point.to_bytes()

    def from_bytes(self, b: bytes) -> PointJacobi:
        return PointJacobi.from_bytes(b)

    def serialize(self, point: PointJacobi) -> str:
        return point.to_hex()

    def deserialize(self, s: str) -> PointJacobi:
        return PointJacobi.from_hex(s)
